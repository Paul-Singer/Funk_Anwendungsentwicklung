...
def parse_dly_annual_and_seasons(dly_path: str, start_year: int, end_year: int, latitude: float):
    ...
    for year in range(start_year, end_year + 1):
        ...
        annual_rows.append(
            {
                "year": year,
                "tmin": _mean_or_none(annual_tmin_month_means, 1),
                "tmax": _mean_or_none(annual_tmax_month_means, 1),
            }
        )
        ...
        for season in ("spring", "summer", "autumn", "winter"):
            ...
            season_rows[season].append(
                {
                    "year": year,
                    "tmin": _mean_or_none(tmin_month_means, 1),
                    "tmax": _mean_or_none(tmax_month_means, 1),
                }
            )

...
function renderDataCoverageHint(rows) {
  ...
  elements.dataCoverageHint.hidden = true;
}
...
function validateForm() {
  ...
  if (!Number.isFinite(values.longitude) || values.longitude < -180 || values.longitude > 180) {
    showError("longitude", "Längengrad muss zwischen -180 und 180 liegen.");
    hasError = true;
  }
  ...
  if (!Number.isInteger(values.endYear) || values.endYear > state.latestYear) {
    showError("endYear", `Endjahr darf nicht größer als ${state.latestYear} sein.`);
    hasError = true;
  }
  if (
...
function normalizeUserFacingErrorMessage(message) {
  if (
    message.includes("Failed to fetch") ||
    message.includes("Connection refused") ||
    message.includes("urlopen error")
  ) {
    return "Datenquelle nicht verfügbar. Versuchen Sie es später erneut.";
  }
  return message;
}
...
function normalizeFetchError(error) {
  ...
  return new Error(normalizeUserFacingErrorMessage(message));
}
...
async function fetchJson(path, params = {}, requestOptions = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(normalizeUserFacingErrorMessage(message));
  }
...
function clearResult() {
  ...
  elements.resultStatus.textContent = "Bitte Station auswählen.";
  if (elements.dataCoverageHint) {
    elements.dataCoverageHint.textContent = "";
    elements.dataCoverageHint.hidden = true;
  }
  ...
  elements.tempEmpty.textContent = "Keine Jahreswerte geladen.";
}
...
function renderStations(stations) {
  ...
  const listHtml = stations
    .map((station) => {
      const distanceValue = Number(station.distanceKm).toFixed(1).replace(".", ",");
      const distance = `${distanceValue} km`;
      ...
      return `
        <li>
          <button type="button" class="station-item${isActive}" data-id="${escapeHtml(station.id)}">
            ...
            <span class="station-meta">${escapeHtml(distance)}</span>
          </button>
        </li>
      `;
    })
...
function showChartPlaceholder(message = "Keine Daten") {
  if (chartInstance) {
    chartInstance.dispose();
    chartInstance = null;
  }
  elements.chartContainer.innerHTML = `<div class="chart-placeholder">${message}</div>`;
}

function resolveChartYearRange(annual) {
...
function buildChartSeriesData(annual, startYear, endYear) {
...
function axisIntervalForRange(range) {
  if (range <= 10) {
    return 0;
  }
  if (range <= 20) {
...
function renderChart(annual) {
  if (!Array.isArray(annual) || annual.length === 0) {
    showChartPlaceholder("Keine Daten");
    return;
  }

  const yearRange = resolveChartYearRange(annual);
  if (!yearRange) {
    showChartPlaceholder("Keine Daten");
    return;
  }
  ...
  const { years, tmin, tmax } = buildChartSeriesData(annual, startYear, endYear);
  ...
  if (!dom || typeof echarts === "undefined") {
    showChartPlaceholder("Diagramm-Bibliothek nicht verfügbar");
    return;
  }
  ...
  const axisInterval = axisIntervalForRange(endYear - startYear);
  ...
  chartInstance.setOption({
    ...
    xAxis: {
      ...
      name: "Jahr",
      nameTextStyle: {
        color: isDark ? "rgba(255,255,255,0.85)" : "rgba(17,24,39,0.88)",
        fontSize: 13
      },
      nameLocation: "middle",
      nameGap: 35,
